...
$(function () {    
    (function ($) {
        $.fn.jalendar = function (options) {
            ...
            $this.find('.jalendar-pages').css({'width' : $this.find('.jalendar-pages').width(260) });
            ...
            $this.on("mouseup",'.this-month',function(e){
                var $th = $(this);
                if(e.which == 1){
                    //左键
                    var eventSingle = $th.find('.event-single');
                    $this.find('.events .event-single').remove();
                    prevAddEvent();
                    $th.addClass('selected').css({'background-color': settings.color});
                }
                if(e.which == 3){
                    //右键
                    $('#calendar_right_menu').menu('show', {
                        left: e.pageX,
                        top: e.pageY
                    });
                    var day = document.getElementById("day");
                    day.value = $th.attr("data-date");
                }
            });
            ...
            // add new event with panel parseInt($this.find('.total-bar b').text());
            /*$this.find('.submit').on('click', function(){
                var date = new Date();
                var dataId = Math.ceil(Math.random()*1000000).toString() + date.getTime();
                ...
                $.ajax({
                    ...
                    success : function(data){
                        data = $.parseJSON(data);
                        if(data.success){
                            $this.prepend(div('div', 'added-event').attr({'data-date':thisDay, 'data-time': time, 'data-title': title, 'data-id': dataId ,'id':dataId}));
            ...
            /*$this.on('click', '.event-single .erase', function(){
                ...
                var data_id = $single.attr("data-id");
                var $myId = $single.parent().parent().parent().parent().parent();
                var th_ = $myId.find("[data-id='"+data_id+"']");
                var id = th_.attr("id");
                $.ajax({
                    type : 'post',
